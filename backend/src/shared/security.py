...
def get_current_user_from_token(token: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise Exception(f"Token validation failed: {str(e)}") from e
...
def verify_verification_token(token: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise Exception("Verification token has expired") from e
    except jwt.InvalidTokenError as e:
        raise Exception("Invalid verification token") from e
