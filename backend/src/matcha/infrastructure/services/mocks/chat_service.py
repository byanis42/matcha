...
class MockChatService:
    """
    Version simulée du service de chat pour les tests.
    """

    def __init__(self, mock_notifications_service):
        ...
        self.notifications_service = mock_notifications_service
        # Pour faciliter les assertions dans les tests
    ...
    async def create_conversation(self, participants: List[str],
        ...
        """
        Crée une nouvelle conversation entre participants (version de test).
        """
    ...
    async def send_message(self, conversation_id: str, sender_id: str,
        ...
        """
        Envoie un message dans une conversation (version de test).
        """
    ...
    async def get_conversation_messages(self, conversation_id: str,
        ...
        """
        Récupère les messages d'une conversation (version de test).
        """
        if conversation_id not in self.conversations:
            return []
    ...
    async def get_user_conversations(self, user_id: str) -> List[Dict[str, Any]]:
        """
        Récupère toutes les conversations d'un utilisateur (version de test).
        """
        if user_id not in self.user_conversations:
            return []
    ...
    async def mark_messages_as_read(self, conversation_id: str, user_id: str) -> int:
        """
        Marque tous les messages comme lus (version de test).
        """
    ...
    def _count_unread_messages(self, conversation_id: str, user_id: str) -> int:
        """
        Compte le nombre de messages non lus (version de test).
        """
        if conversation_id not in self.conversations:
            return 0
    ...
    async def add_participants(self, conversation_id: str, user_ids: List[str]) -> bool:
        """
        Ajoute des participants à une conversation (version de test).
        """
        if conversation_id not in self.conversations:
            return False
    ...
    async def leave_conversation(self, conversation_id: str, user_id: str) -> bool:
        """
        Permet à un utilisateur de quitter une conversation (version de test).
        """
        if (conversation_id not in self.conversations or
            user_id not in self.conversation_participants[conversation_id]):
            return False
